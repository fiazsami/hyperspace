 ...
 * test_impknot.cpp's copy of it. Its 3x3 linear part happens to be
 * symmetric ([[2,1,1],[1,3,1],[1,1,4]]), which makes its own inverse
 * symmetric too -- but setMatrix's transpose step also relocates the
 * translation column into the bottom row (and the all-zero bottom row into
 * the translation-of-inverse slot), and those two blocks are not
 * symmetric to each other. So this matrix still lets
 * setMatrix_copies_mat_and_builds_invtrmat_as_the_transpose_of_invmat catch
 * a verbatim invtrmat=invmat copy that skips the relocation, even though
 * its linear part alone would not.
 *
 * invmat's linear part was solved independently as the analytic inverse of
 * that 3x3 (determinant 17): [[11,-3,-2],[-3,7,-1],[-2,-1,5]]/17. Its
 * translation column is -R^-1 * (1,2,3) = (1/17, -8/17, -11/17). */
 ...
 * no-op setPosition would leave the coupled matrix's own (1/17, -8/17,
 * -11/17) sitting there instead of being overwritten. */
...
TEST(invertMatrix_inverts_the_coupled_matrix)
{
    ...
    CHECK(ok);
    CHECK_NEAR(s.invmat[0], 11.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[1], -3.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[2], -2.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[4], -3.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[5], 7.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[6], -1.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[8], -2.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[9], -1.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[10], 5.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[12], 1.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[13], -8.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invmat[14], -11.0f / 17.0f, 1e-5f);
 ...
 * linear part is symmetric (see makeCoupledMatrix's comment), so that block
 * alone can't distinguish a real transpose from a verbatim copy -- but the
 * translation/bottom-row swap can: a verbatim invtrmat=invmat copy would
 * leave invtrmat[3]=invmat[3]=0 and invtrmat[12]=invmat[12]=1/17, the
 * opposite of what setMatrix is supposed to produce. */
...
TEST(setMatrix_copies_mat_and_builds_invtrmat_as_the_transpose_of_invmat)
{
    ...
    /* Linear block: transposing a symmetric block reproduces the same
     * values at the same indices, so this confirms the block was populated
     * from invertMatrix's real output rather than left uninitialised. */
     ...
     * backwards. */
    CHECK_NEAR(s.invtrmat[3], 1.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invtrmat[7], -8.0f / 17.0f, 1e-5f);
    CHECK_NEAR(s.invtrmat[11], -11.0f / 17.0f, 1e-5f);
